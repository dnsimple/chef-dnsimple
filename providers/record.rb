...
def load_current_resource
  @current_resource =
    Chef::Resource::DnsimpleRecord.new(@new_resource.record_name)
...
def create_record
  Chef::Log.debug "Attempting to create record type #{new_resource.type} " \
                  "for #{new_resource.record_name} as " \
                  "#{new_resource.content} with type #{new_resource.type}"

  zone = dnsimple.zones.get(new_resource.domain)
  zone.records.create(name: new_resource.record_name,
                      value: new_resource.content,
                      type: new_resource.type,
                      ttl: new_resource.ttl)
  new_resource.updated_by_last_action(true)
  Chef::Log.info "DNSimple: created #{new_resource.type} record for " \
                 "#{new_resource.record_name}.#{new_resource.domain}"
rescue Excon::Errors::UnprocessableEntity
  Chef::Log.debug "DNSimple: #{new_resource.record_name}." \
                  "#{new_resource.domain} already exists, moving on"
end
  ...
  if [:same, :different].include?(@current_resource.exists)
    all_records_in_zone do |r|
      if (r.name == new_resource.record_name) && (r.type == new_resource.type)
        r.destroy
        new_resource.updated_by_last_action(true)
        Chef::Log.info "DNSimple: destroyed #{new_resource.type} record for " \
                       "#{new_resource.record_name}.#{new_resource.domain}"
      end
    end
  else
    Chef::Log.info "DNSimple: no record found #{new_resource.record_name}." \
                   "#{new_resource.domain} with type #{new_resource.type}"
  end
end
